#!/usr/bin/env python3
"""Build and update the standalone Guide Browser artifact in guides/index.html.
...
GUIDE_FILE_PATTERN = re.compile(r"^(\d{2})-([A-Z]+)-(.+)\.md$")
...
def build_tags(guide_type: str, slug: str, title: str) -> list[str]:
    tags = [guide_type.lower()] + [part for part in slug.split("-") if part]
    title_lower = title.lower()
    if "git" in title_lower:
        tags.append("git")
    if "latex" in title_lower or "overleaf" in title_lower:
        tags.extend(["latex", "overleaf"])
    if "spec" in title_lower or "speckit" in title_lower:
        tags.extend(["spec-kit", "sdd"])
    if "superpowers" in title_lower:
        tags.extend(["superpowers", "tdd"])
...
def scan_guides(guides_dir: Path) -> list[dict]:
    guides = []
    md_files = sorted(guides_dir.glob("*.md"), key=lambda p: p.name)

    for md_path in md_files:
        match = GUIDE_FILE_PATTERN.match(md_path.name)
        if not match:
            continue

        guide_id, guide_type, slug = match.groups()
        content = md_path.read_text(encoding="utf-8")

        # Extract title from first H1
        h1_match = H1_PATTERN.search(content)
        title = h1_match.group(1).strip() if h1_match else f"{guide_type}: {slug}"

        # Extract summary from first blockquote
        bq_match = BLOCKQUOTE_PATTERN.search(content)
        summary = bq_match.group(1).strip() if bq_match else ""

        # Check companion SVG
        svg_name = f"{guide_id}-{guide_type}-{slug}.svg"
        svg_path = guides_dir / svg_name
        has_svg = svg_path.is_file()
        svg_content = svg_path.read_text(encoding="utf-8") if has_svg else ""

        tags = build_tags(guide_type, slug, title)
        ...
        guide_obj = {
            ...
            "title": title,
            "summary": summary,
            "mdFilename": md_path.name,
            "svgFilename": svg_name,
            "hasSvg": has_svg,
            "tags": tags,
            "contentMarkdown": content,
            "contentSvg": svg_content,
        }
        guides.append(guide_obj)
        print(f"  -> Cataloged [{guide_id}] {guide_type}: {title} (SVG: {has_svg})")

    return guides


def build_catalog(guides: list[dict]) -> dict:
    return {
        "version": "1.0.0",
...
def compile_guide_browser(guides_dir: Path, output_file: Path) -> int:
    print(f"Scanning guides in: {guides_dir}")
    guides = scan_guides(guides_dir)

    catalog = build_catalog(guides)
    catalog_json = json.dumps(catalog, ensure_ascii=False, separators=(",", ":"))
    catalog_json_safe = catalog_json.replace("</script>", r"<\/script>")

    final_html = HTML_TEMPLATE.replace("__CATALOG_JSON_DATA__", catalog_json_safe)

    output_file.parent.mkdir(parents=True, exist_ok=True)
    output_file.write_text(final_html, encoding="utf-8")

    print(f"Successfully generated Guide Browser at: {output_file}")
...
def main() -> int:
    ...
    default_guides_dir = script_dir.parent / "guides"
    default_output_file = default_guides_dir / "index.html"

    parser = argparse.ArgumentParser(description="Build standalone Guide Browser artifact.")
    parser.add_argument(
        "--guides-dir",
        type=Path,
        default=default_guides_dir,
        help=f"Directory containing canonical guides (default: {default_guides_dir})",
    )
    parser.add_argument(
        "--output-file",
        type=Path,
        default=default_output_file,
        help=f"Destination HTML file (default: {default_output_file})",
    )

    args = parser.parse_args()
    return compile_guide_browser(args.guides_dir, args.output_file)
